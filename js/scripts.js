...
function render()
{
    writeTable(pTable.table);
    writeRow(pTable.lanthanoids);
    writeRow(pTable.actinoids);
    addListeners();
}
...
function writeRow(row)
{
    ...
    {
    	while(row[element].position > spacesCounter)
    	{
    		data += "<td></td>";
    		spacesCounter++;
    	}
    	data += "<td class = \"element\" data-name=\"" + row[element].name + "\" data-number=" + row[element].number + " data-mass=" + row[element].molar + " data-symbol=" + row[element].small+ ">" + row[element].small + "<sup> " + row[element].number+ "</sup></td>";
    	spacesCounter++;
    }
    insert.html(data)
    $("#ptable").append(insert);
}
function addListeners()
{
	$(".element").click(function(){
		...
		{
			masses.push(this.getAttribute("data-mass"));
			elements.push(this.getAttribute("data-symbol"));
			update();
		}
 		});
 	$( ".element" ).hover(
  function() {
    $( this ).append( $( "<span><sub style=\"font-size: 50%;\">" +this.getAttribute("data-name")+"</sub></span>" ) );
  }, function() {
...
function update()
{
	if(masses.length !== 0 && elements.length !== 0)
{
		var elementsCompressed = compressArray(elements);
		var mass = 0;
		var text = "";
		...
		for(var i=0; i < elementsCompressed.length; i++)
		{
			text += elementsCompressed[i].value; 
			if(elementsCompressed[i].count != 1)
				text += "<sub>" + elementsCompressed[i].count + "</sub>";
		}
		text += " =" + mass + " g/mol";
		$("#add-display").html(text);
	}
	else
	{
		$("#add-display").html("");
...
//from https://gist.github.com/ralphcrisostomo/3141412#file-array_dupplicate_counter-js
